...
export async function listAdminOrders(page: number, limit: number, search?: string, status?: string): Promise<Page<AdminOrderView>> {
  ...
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : ''
  const count = await pool.query<{ count: string }>(`SELECT COUNT(*)::text AS count FROM orders o JOIN users au ON au.id = o.user_id LEFT JOIN order_items oi ON oi.order_id = o.id LEFT JOIN ticket_types tt ON tt.id = oi.ticket_type_id LEFT JOIN events e ON e.id = tt.event_id ${where}`, values)
  values.push(limit, (page - 1) * limit)
