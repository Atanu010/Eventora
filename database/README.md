...
Phase 6 integrates Razorpay test mode through server-side order creation, signature verification, webhook idempotency, and locked payment confirmation. Phase 7 adds server-authoritative ticket check-in. Phase 8 stores durable in-app notifications with deduplicated lifecycle hooks, read state, and retry processing through `npm run notifications:process`. Phase 9 adds organizer dashboards backed by authoritative sales, inventory, and check-in aggregates. Phase 10 adds the `audit_logs` table and admin platform management APIs for moderation, operational monitoring, and immutable audit inspection. Tickets are generated only after the Eventora backend confirms payment. Refunds remain future work.

Phase 11 adds no migration. Operational readiness uses the existing PostgreSQL pool and `GET /api/ready`; configuration is supplied through server-only environment variables documented in `.env.example`.

Phase 12 adds migration `007_refunds.sql` and a full-refund-only Razorpay workflow. A paid confirmed order may be refunded by its attendee or an admin only when no ticket has been checked in. The refund locks the order/payment/ticket inventory, invalidates active tickets, restores reserved inventory, updates payment/order state, creates notifications, and records the initiating actor in the audit log. Partial refunds are explicitly rejected because the current ticket model does not safely map arbitrary money amounts to ticket quantities.
